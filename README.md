...
### 3. Obtain Excluded Files

The following files are **not included in the repository** (excluded via `.gitignore`) because they are either large datasets or generated model artifacts. Follow the steps below to get each one.

---

#### 📂 A. Jigsaw Toxic Comment Dataset

> **Where it goes:** `backend/model/Gigsaw Dataset/train.csv`

1. Go to the [Jigsaw Toxic Comment Classification Challenge on Kaggle](https://www.kaggle.com/c/jigsaw-toxic-comment-classification-challenge/data).
2. Accept the competition rules and click **Download All**.
3. Extract the ZIP — you need `train.csv` (~67 MB).
4. Create the folder and place the file:
   ```bash
   mkdir -p backend/model/Gigsaw\ Dataset
   mv /path/to/train.csv backend/model/Gigsaw\ Dataset/train.csv
   ```

> **Kaggle CLI alternative:**
> ```bash
> pip install kaggle
> kaggle competitions download -c jigsaw-toxic-comment-classification-challenge -p backend/model/Gigsaw\ Dataset/
> unzip backend/model/Gigsaw\ Dataset/jigsaw-toxic-comment-classification-challenge.zip -d backend/model/Gigsaw\ Dataset/
> ```

---

#### 📂 B. HateXplain Dataset

> **Where it goes:** `backend/model/HateXplain_dataset.json`

```bash
wget -O backend/model/HateXplain_dataset.json \
  https://raw.githubusercontent.com/hate-speech-and-offensive-language/HateXplain/master/Data/dataset.json
```

Or download manually from the [HateXplain GitHub repo](https://github.com/hate-speech-and-offensive-language/HateXplain/blob/master/Data/dataset.json) and save it as `backend/model/HateXplain_dataset.json`.

The `HateXplain_classes.npy` file is auto-generated by `train_model.py` — you do **not** need to download it separately.

---

#### 🤖 C. Trained Model Files (`.pkl`) and other generated artifacts

> **Files:** `toxic_model.pkl`, `vectorizer.pkl`, `tfidf_vectorizer.pkl`, `merged_dataset.csv`, `threshold.txt`, `synthetic_soft_insults.csv`

These are all **generated** by running the training scripts — you do **not** download them from anywhere. After placing the datasets above, run steps 4 and 5 to produce them automatically.

> 💡 **Shortcut (skip training):** If a collaborator shares the pre-trained `.pkl` files with you (e.g., via Google Drive or a release attachment), simply place them in `backend/model/` and jump straight to Step 6.
...
This creates `backend/model/synthetic_soft_insults.csv` (~420 hand-curated indirect insult examples).
...
Training takes approximately 3–5 minutes. It reads all three datasets, merges them, trains the classifier, and writes:

| Output file | Description |
|---|---|
| `model/toxic_model.pkl` | Full model payload (word vec + char vec + classifier) |
| `model/vectorizer.pkl` | Standalone word TF-IDF vectorizer |
| `model/tfidf_vectorizer.pkl` | Character TF-IDF vectorizer |
| `model/merged_dataset.csv` | Final merged training dataset (~180k rows) |
| `model/threshold.txt` | Best classification threshold (e.g. `0.5`) |
